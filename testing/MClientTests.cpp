...
/* Aggiungi strumento DAX daily */
    ...
    int estx_hourly_id = client->add_Instrument("1 hour", MContractDetails::ESTX50Index(), MContractDetails::ESTX50Future(), 
                                    Instrument::ReqIds(5101, 5201, 5301, 5401, 5501), "../instruments_log/Estx_hourly.txt");
    /* int nikkei_hourly_id = client->add_Instrument("1 hour", MContractDetails::NIKInd(), MContractDetails::NIKFut(),
                                    Instrument::ReqIds(6101, 6201, 6301, 6401, 6501), "../instruments_log/Nikkei_hourly.txt"); */
...
/* Inizializza gli indicatori per ogni strategia, e per ogni grandezza di barre (4 blocchi in totale)
   ...
   che poi verranno prese in considerazione per dare i vari segnali di entrata e uscita */
    ...
    Instrument * estx_hourly_instr = client->get_Instrument(estx_hourly_id);
    /* Instrument * nikkei_hourly_instr = client->get_Instrument(nikkei_hourly_id); */
    /* Determina barre hourly e daily da includere nei backtest */

    int hourly_inst_id = estx_hourly_id;
    Instrument * hourly_instr = estx_hourly_instr;

    const int hourly_bars = 20000;
    const int daily_bars  = 5000;
    ...
    const double S1_take_profit_hourly = 4.0;
    const double S1_stop_loss_hourly   = 3.0;
    const double S2_take_profit_hourly = 3.0;
    const double S2_stop_loss_hourly   = 3.0;
    ...
    const BOLLINGER_CONDITION Boll_cond = MIDDLE_BAND;
    /* Indicatori per S1 hourly */
    Indicators::LocalMin *       S1_hourly_LocalMin       = new Indicators::LocalMin      (hourly_instr->bars); 
    Indicators::LocalMax *       S1_hourly_LocalMax       = new Indicators::LocalMax      (hourly_instr->bars);
    Indicators::RSI      *       S1_hourly_RSI            = new Indicators::RSI           (hourly_instr->bars);
    Indicators::BollingerBands * S1_hourly_BollingerBands = new Indicators::BollingerBands(hourly_instr->bars, 2.0, 2.0, 14); // NOTA: S1 utilizza le Bollinger a 2.0
        //!! Separate indicators because these are the only indicators changed by each strategy (by marking denied divergences!)
    Indicators::Divergence *     S1_hourly_Divergence0     = new Indicators::Divergence    (hourly_instr->bars, S1_hourly_LocalMax, S1_hourly_LocalMin, S1_hourly_RSI, min_div_period, max_hourly_div_period);
    Indicators::LongDivergence * S1_hourly_LongDivergence0 = new Indicators::LongDivergence(hourly_instr->bars, S1_hourly_LocalMax, S1_hourly_LocalMin, S1_hourly_RSI, min_div_period, max_hourly_div_period);
    Indicators::Divergence *     S1_hourly_Divergence1     = new Indicators::Divergence    (hourly_instr->bars, S1_hourly_LocalMax, S1_hourly_LocalMin, S1_hourly_RSI, min_div_period, max_hourly_div_period);
    Indicators::LongDivergence * S1_hourly_LongDivergence1 = new Indicators::LongDivergence(hourly_instr->bars, S1_hourly_LocalMax, S1_hourly_LocalMin, S1_hourly_RSI, min_div_period, max_hourly_div_period);
    /* Indicatori per S2 hourly */
    Indicators::LocalMin *       S2_hourly_LocalMin       = new Indicators::LocalMin      (hourly_instr->bars); 
    Indicators::LocalMax *       S2_hourly_LocalMax       = new Indicators::LocalMax      (hourly_instr->bars);
    Indicators::RSI      *       S2_hourly_RSI            = new Indicators::RSI           (hourly_instr->bars);
    Indicators::BollingerBands * S2_hourly_BollingerBands = new Indicators::BollingerBands(hourly_instr->bars, 2.5, 2.5, 14); // NOTA: S2 utilizza le Bollinger a 2.5
    Indicators::Divergence *     S2_hourly_Divergence     = new Indicators::Divergence    (hourly_instr->bars, S2_hourly_LocalMax, S2_hourly_LocalMin, S2_hourly_RSI, min_div_period, max_hourly_div_period);
    Indicators::LongDivergence * S2_hourly_LongDivergence = new Indicators::LongDivergence(hourly_instr->bars, S2_hourly_LocalMax, S2_hourly_LocalMin, S2_hourly_RSI, min_div_period, max_hourly_div_period);
...
/* Utilizzando gli strumenti, le condizioni, e gli indicatori definiti qua sopra,
   inizializziamo le varie strategie (S1 e S2 daily e hourly) */
    ...
    /* Strategy * S1_daily           = new Strategy(dax_daily_instr, client->m_tradeData, 
                                    S1_take_profit_daily, S1_take_profit_daily, S1_exp_bars_daily, 
                                    ...
                                    LONG, max_daily_neg_period, NONE
                                    ); */
    Strategy * S1_hourly_shortDiv = new Strategy(hourly_instr, client->m_tradeData,
                                    S1_stop_loss_hourly, S1_take_profit_hourly, S1_exp_bars_hourly, /* Settare qua stop loss, take profit, e massime barre prima della chiusura in negativo*/
                                    ...
                                    SHORT, max_hourly_neg_period, RSI_cond, Boll_cond /* parametri per divergenza negata */
                                    );
    Strategy * S1_hourly_longDiv = new Strategy(hourly_instr, client->m_tradeData, 
                                    S1_stop_loss_hourly, S1_take_profit_hourly, S1_exp_bars_hourly, /* Settare qua stop loss, take profit, e massime barre prima della chiusura in negativo*/
                                    ...
                                    LONG, max_hourly_neg_period, RSI_cond, Boll_cond /* parametri per divergenza negata */
                                    );

    Strategy * S2_hourly = new Strategy(hourly_instr, client->m_tradeData,
                                    S2_stop_loss_hourly, S2_take_profit_hourly, S2_exp_bars_hourly, /* Settare qua stop loss, take profit, e massime barre prima della chiusura in negativo*/
                                    ...
                                    LONG, max_hourly_neg_period, RSI_cond, Boll_cond  /* Divergeze lunghe sembrano funzionare molto meglio su S2*/
                                    );
                                
    /* Strategy * S2_daily = new Strategy(dax_daily_instr, client->m_tradeData, 
                                    S2_take_profit_daily, S2_take_profit_daily, S2_exp_bars_daily, 
...
/* Aggiungiamo le strategie */
    client->add_Strategy(hourly_inst_id, S1_hourly_shortDiv);
    client->add_Strategy(hourly_inst_id, S1_hourly_longDiv);
    client->add_Strategy(hourly_inst_id, S2_hourly);
...
/* Esegui backtesting utilizzando i dati degli ultimi 3 anni */
    ...
    client->update_instr_bars(hourly_inst_id, hourly_bars, true);
